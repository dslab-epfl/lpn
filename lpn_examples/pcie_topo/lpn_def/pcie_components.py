...
from pcie_topo.lpn_def.all_enum import *
...
from DMA.lpn_def.dma_ports import DMAPortBase
...
class PcieDevice(PcieBase):
    '''
    A request token should have the following fields:
        Token({"dst":, "src":, "type":, "pkt_size", "offset", "dataref":})"
    dataref will contain:
        request size for read, request address, request type, etc
        if a request is write(or completion) type, the pkt_size and offset will contain the actual position of the data to be written
            request_size in dataref should not be used in this case
        if a request is read type, the pkt_size will contain the pkt size is always 16
            the offset should not be used as well.  
    
    this token only contains necessary information for interconnect simulation

    A device has request buffer post and nonpost, and completion buffer
        post buffer will not receive response
        nonpost buffer will receive response
    '''

    def __init__(self, id, fair=True):
        self.id = id
        'because the token only takes int as the id'
        seq_num = int(id[-1])
        self.fair = fair
        ...
        self.recv_cmpl = Place(f"{self.id}_recvcmpl")
        ...
        self.process.p_output.append(self.recv_cmpl)
        self.process.po_w.append(store_cmpl(self.in_buf))
        self.process.p_output.append(self.recv_req)
        self.process.po_w.append(store_request(self.in_buf))
        ...
        """
        p_list = [self.in_buf, self.in_buf_cap, self.recv_cmpl, self.recv_req, 
        ...
        self.out_buf_cap]
        t_list = [self.out_trans, self.process, self.arbit_p_np]
        self.t_list = t_list
        self.node_dict = {}
        for p in p_list:
            self.node_dict[p.id] = p
...
class MemDevice(PcieDevice):
    def __init__(self, id, fair=True):
        super().__init__(id, fair)
        dma_read = DMAPortBase(f"{self.id}_mem_read", 0, 1)
        dma_write = DMAPortBase(f"{self.id}_mem_write", 1, 1)
        dma_read.init_places()
        dma_write.init_places()

        read_put_port_0, read_get_port_0, _, _ = dma_read.port(0)
        
        write_put_port_0, write_get_port_0, _, _ = dma_write.port(0)

        self.send_mem_req = Transition(id=f"{self.id}_send_mem_req")

        self.mem_req_max_inflight = Place(f"{self.id}_mem_req_max_inflight")
        self.mem_req_max_inflight.assign_marking(create_empty_tokens(2))

        'mem latency is handled by mem simulator, not by me'
        self.send_mem_req.delay_f = con_delay(0)
        self.send_mem_req.p_input.append(self.recv_req)
        self.send_mem_req.pi_w.append(take_1_token())

        self.send_mem_req.p_input.append(self.mem_req_max_inflight)
        self.send_mem_req.pi_w.append(take_1_token())

        'break here to include mem device simulation'
      
        self.send_mem_req.p_output.extend([read_put_port_0, write_put_port_0])
        self.send_mem_req.po_w.extend([mem_request_read(self.recv_req, MPS, 0), mem_request_write(self.recv_req, MPS, 0)])

        resp_buf = Place(f"{self.id}_resp_buf")
        self.get_read = Transition(id=f"{self.id}_get_read")
        self.get_read.delay_f = con_delay(0)
        self.get_read.p_input.append(read_get_port_0)
        self.get_read.pi_w.append(take_1_token())
        self.get_read.p_output.append(resp_buf)
        self.get_read.po_w.append(pass_token(read_get_port_0, 1))
        
        self.get_write = Transition(id=f"{self.id}_get_write")
        self.get_write.delay_f = con_delay(0)
        self.get_write.p_input.append(write_get_port_0)
        self.get_write.pi_w.append(take_1_token())
        self.get_write.p_output.append(resp_buf)
        self.get_write.po_w.append(pass_token(write_get_port_0, 1))

        self.send_cmpl_reply = Transition(id=f"{self.id}_send_cmpl_reply")
        self.send_cmpl_reply.delay_f = con_delay(0)
        self.send_cmpl_reply.p_input.append(resp_buf)
        self.send_cmpl_reply.pi_w.append(take_1_token())
        self.send_cmpl_reply.p_input.append(self.req_buf_cmpl_cap)
        self.send_cmpl_reply.pi_w.append(take_1_token())
        self.send_cmpl_reply.p_output.append(self.req_buf_cmpl)
        self.send_cmpl_reply.po_w.append(send_cmpl(resp_buf))

        self.send_cmpl_reply.p_output.append(self.mem_req_max_inflight)
        self.send_cmpl_reply.po_w.append(pass_empty_token(1))
        
        if not self.fair:
            self.send_cmpl_reply.p_output.append(self.order_keeper)
            self.send_cmpl_reply.po_w.append(pass_type_idx(self.req_buf_map, CstStr.CMPL))
    
        self.t_list.extend([
                        self.get_read, self.get_write,
                        self.send_mem_req,
                        self.send_cmpl_reply])

        self.t_list.extend(dma_read.transitions())
        self.t_list.extend(dma_write.transitions())

        self.node_dict[self.send_mem_req.id] = self.send_mem_req
        self.node_dict[self.send_cmpl_reply.id] = self.send_cmpl_reply