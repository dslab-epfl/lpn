...
this is an arbiter based on ordering, the order is provided explictly
"""
def arbiterWTimeOrdHelperNoCap(id, list_of_buf, out_buf, porder_keeper):
    total = len(list_of_buf)
    take_0_or_1 = InWeightFunc(f"arbiterhelperord_take_0_or_1", my_idx=int, porder_keeper=Place)
    @take_0_or_1.install    
    def weight(binding, my_idx, porder_keeper):
    ...
    arbiter = Transition(
        id = f"{id}_arbiter",
        delay_func=con_delay(0),
        pi=[porder_keeper, *list_of_buf],
        pi_w=[take_1_token(), *[take_0_or_1(idx, porder_keeper) for idx in range(total)] ], 
        po=[out_buf],
        po_w=[pass_turn_token(porder_keeper, list_of_buf)],
    )
