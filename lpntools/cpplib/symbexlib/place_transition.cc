...
uint64_t simple_max(uint64_t a, uint64_t b){
  return a > b ? a:b;
}
...
int able_to_fire_t(Transition* self, uint64_t& enabled_ts){
  ...
  for(int i = 0; i< input_size; i++){ 
      ...
      if(self->pi_w_threshold[i] != NULL){
          int threshold = self->pi_w_threshold[i]();
          if(threshold != 0){
            consume_num_tokens_threshold = threshold; 
          }
      } 
      ...
      max_ts = simple_max(max_ts, p->tsAt(consume_num_tokens_threshold-1)); 
  }
  enabled_ts = max_ts;
  return 1;
} 
...
int trigger(Transition* self){
  ...
  if(self->delay_event == lpn::LARGE && can_fire){
     ...
     uint64_t enable_time = simple_max(enabled, self->pip_ts);
