    ...
    def output_tokens(binding, output_place, pidx, list_of_buf, in_post_list):
        ...
        for i in _l:
            if tk_status[i] > 0:
                ...
                break
        token = Token({
            "dst": prop_value(binding, cur_buf, 0, "dst"),
            "src": prop_value(binding, cur_buf, 0, "src"),
            "type": prop_value(binding, cur_buf, 0, "type"),
            "pkt_size": prop_value(binding, cur_buf, 0, "pkt_size"),
            "offset": prop_value(binding, cur_buf, 0, "offset"),
            "dataref": prop_value(binding, cur_buf, 0, "dataref"),
            "port": cur_port
        })
        output_place.push_token(token)
