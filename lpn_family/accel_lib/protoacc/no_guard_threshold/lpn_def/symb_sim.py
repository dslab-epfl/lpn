...
from lpn_def.testing.symb_parse_message import *
...
def symbolic_setup(p_list, t_list, node_dict, messages):
    for p in p_list:
        p.reset()
    for t in t_list:
        t.reset()
    ...
    print(len(symb_control_tokens))
    print(len(symb_fields_meta_tokens))
    print(len(symb_unit_tokens))
    pmessage_tasks.assign_marking(symb_control_tokens)
    pfields_meta.assign_marking(symb_fields_meta_tokens)
    pfields.assign_marking(symb_unit_tokens)
