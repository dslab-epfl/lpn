...
from sympy import nsimplify, symbols, Eq, solve, Add, Max, Min, refine, simplify, Symbol, S, solve, Interval, Union, Expr, collect, Wild, srepr, sympify
from itertools import product
import json
import numpy as np
from place_transition import IntSymbol, Token
...
def extend_binding(binding, p, tokens):
    ...
    keys = tokens[0].dir.keys()
    for i in range(len(tokens)):
        for key in keys:
            binding[f"{p.id}.{i}.{key}"] = tokens[i].dir[key]
    # not used
    return binding
        ...
        for i, cycle in enumerate(unique_cycles):
            ...
            while 1: 
                ...
                for binding in bindings:
                    delay = transition.delay_f(binding)
                    pip_delay = transition.pip(binding)
                    input_index = transition.p_input.index(node_dict[input_place_id])
                    input_weight = transition.pi_w[input_index](binding)  
                    
                    if transition.pi_w_threshold != None and transition.pi_w_threshold[input_index] > 0:
                        if input_weight < transition.pi_w_threshold[input_index]:
                            input_weight = transition.pi_w_threshold[input_index]

                    output_index = transition.p_output.index(node_dict[cycle[(cycle_index)%cycle_len]])
                    output_tokens = transition.po_w[output_index](binding)
            ...
            for all_pairs in product(*all_weights):
                ...
                while 1:
                    ...
                    if True: #transition.pip == -1: 
                        ...
                        print("cycle_delay_1 ", cycle_delay_part_1)
                        print("cycle_delay_2 ", cycle_delay_part_2)
                        # cycle_delay_part_1 += 0
    ...
    for t in t_list:
        if len(t.nonloop_stall_list) == 0 and t.count != 0:
            ...
            for binding in bindings:
                ...
                if delay != -1:
                    triggered = True
                    sum_of_pip_delay += delay

            # if triggered:
            #     sum_of_delay = sum_of_pip_delay

            # if t.pip != -1:
            #     sum_of_delay = len(binding)*t.pip

            if isinstance(sum_of_delay, Expr):
                print("expr   : ", sum_of_delay)
