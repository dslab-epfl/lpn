...
def setup(comps, node_dict, benchmark_file):

    all_inst = collect_insns(benchmark_file)
    numInst = deque() 
    numInst.append(Token({"total_insn":len(all_inst)}))
    node_dict["plaunch"].assign_marking(numInst)
    node_dict["pnumInsn"].assign_marking(all_inst)
