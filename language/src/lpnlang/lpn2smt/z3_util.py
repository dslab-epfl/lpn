...
def makePosIntVar(sol):
    v = Int(getNewId())
    sol.add(v >= 0, v <= 10000000)
    return v
...
def maximum2(sol, x):
  assert( len(x) !=0 )
  v = Int("vmaximum_%i"% uuid.uuid4().int)
  sol.add(Or([v == x[i] for i in range(len(x))])) # v is an element in x)
  for i in range(len(x)):
    sol.add(v >= x[i]) # and it's the greatest
  return v

def maximum2_real(sol, x):
  # v = makeIntVar(sol,"v_%i"% uuid.uuid4().int, min_val, max_val)
  v = Real("v_%i"% uuid.uuid4().int)
  sol.add(Or([v == x[i] for i in range(len(x))])) # v is an element in x)
  for i in range(len(x)):
    sol.add(v >= x[i]) # and it's the greatest
  return v
